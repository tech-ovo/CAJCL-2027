...
**Connections are pooled per thread, not opened per transaction.** Opening one costs a TLS handshake to Turso — measured at roughly 350 ms from the browser — and an authenticated request needs two: the guard reads the session, then the handler reads the data. A thread-local pool means the second finds the first idle and takes it, so a warm request opens one connection instead of two. A connection is never shared between threads, never handed to two transactions at once, and closed by its own thread when that thread retires. `DB_POOL=0` in the Modal secret restores connection-per-transaction without a deploy; `backend/lib/db.py` explains the rest.
...
It exposes five operations — `upload`, `fetch`, `list`, `mkdir`, `trash` — and holds no configuration. `trash` moves a file to Drive's trash, which Google empties after 30 days; nothing here deletes permanently. Folder IDs, filenames, and retention rules all travel in the request payload from Modal. The **one** secret that must live in Script Properties is a shared HMAC key, because the web app is deployed as "execute as me / anyone with the link" and needs some way to verify the caller is actually your backend. Requests carry an HMAC signature over `ts.op.folderId.name.fileId` and a timestamp; the script rejects anything older than five minutes. Until the convention Workspace account exists it runs on a personal Gmail, which carries no data-processing agreement — see [`PRIVACY.md` §2.1](PRIVACY.md#21-the-four-platforms-and-what-each-has-certified).
