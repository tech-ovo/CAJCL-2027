...
**Pre-convention contests** — Digital Art/Poster, Modern Myth, Original Poetry, Original Slogan (English), Original Slogan (Latin), and the chapter's Publicity portfolio — are submitted **through the website**, from a delegate's *Contests* tab and a sponsor's (or chapter leader's) *Contests*/*Publicity* tab. Slogans are typed in; Publicity is a link to a shared Google document plus the categories it includes; the rest are files. A file goes to Modal, Modal passes it to the Apps Script puppet, and the puppet files it into the Google Drive under a folder structure organized by contest and then by chapter, returning the Drive file ID for Modal to store alongside the entry. Word files and text files are also read for their words, so a myth's length is counted rather than claimed; a PDF's length is declared by the student. Each entry is placed in its division when it arrives: Digital Art/Poster by Latin level, in exactly seven divisions (MS-I, MS-II, MS-III, HS-I, HS-II, HS-III, HS-Advanced); Myth, Poetry and the slogans by MS, HS 9–10 and HS 11–12; Publicity by MS and HS. Entries can be replaced or withdrawn until the pre-convention deadline, a dashboard setting.

**Judging happens on the site, blind.** Anyone holding the *Contest Judge* role (scope `judge`) opens *Judging*, reads each entry as "Entry 14" with no name, chapter or original file name, and **ranks their top N** in each division — for Publicity, in each category — where N is set per contest (three unless the chairs change it; a division with fewer entries ranks them all). The Myth length penalty is shown next to each entry's word count for the judge to weigh. A judge can save a partial list as a draft and hand it in later; only handed-in lists count. Files are fetched back through Modal rather than shared from Drive, because a chapter's Drive folder says whose the work is. If a student replaces or withdraws an entry, it comes off every judge's list and those lists go back to draft. **The Academics chairs do not judge** — the server refuses the judging screens to anyone holding `academics`, even alongside the judge role — because they are the ones who see names. The Academics and Awards chairs see *Results* (`#/contest-results`): a judge's 1st is worth N points down to 1 for their Nth, points are summed across judges, equal points go to whoever has more 1sts (then 2nds, …), and entries still level share a place; names, chapters and each judge's place are shown, and anybody no longer attending is listed but not placed (Publicity excepted). The Academics chairs edit each contest's rules text and N on a contest's *Rules and places* tab (divisions are fixed by the Convention Book). N may change after judging: a longer list counts only its first N, and a shorter one counts as it stands and shows its judge that it needs more.
