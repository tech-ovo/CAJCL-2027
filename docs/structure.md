...
**Judging happens on the site, blind.** Anyone holding the *Contest Judge* role (scope `judge`) opens *Judging*, reads each entry as "Entry 14" with no name, chapter or original file name, and scores it against the contest's rubric, one sheet per Publicity category. A judge can save a draft and hand the score in later; only handed-in scores count. Files are fetched back through Modal rather than shared from Drive, because a chapter's Drive folder says whose the work is. **The Academics chairs do not judge** — the server refuses the judging screens to anyone holding `academics`, even alongside the judge role — because they are the ones who see names. The Academics and Awards chairs see *Contest results*: each division ranked by the mean of its judges' totals, with names and chapters, the Myth length penalty applied, and anybody no longer attending listed but not placed (Publicity excepted). The Academics chairs edit each contest's rules text and rubric from the same page (divisions are fixed by the Convention Book); once anyone has scored, the rubric can be reworded but not reshaped.
